...
(**********************************************************)
      ...
      let mk_addr proc addr =
        let lbls = labels_of_instr addr in
        let lbl = Label.norm lbls in
        let addr_v = 
          match lbl with
          | Some l -> A.V.cstToV (Constant.mk_sym_virtual_label proc l)
          | None ->  A.V.intToV addr in
        A.Location_global addr_v in
